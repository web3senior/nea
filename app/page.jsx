...
import Croak from './assets/croak.png'
import LSP7ABI from './abi/erc20.json'
...
export default function Home() {
  ...
  const sendMessage = (e) => {
    ...
    const raw = JSON.stringify({
      profile: {
        role: 'user',
        content: `Connected wallet address ${JSON.stringify(auth.wallet)}`,
      },
    ...
    <div className={styles.page}>
      <header className={styles.header}>
        ...
        <div className={`d-flex grid--gap-1`}>
          <a className={`${styles.link} d-flex grid--gap-050`} href={`https://croakonlinea.eth.limo/`} target="_blank" rel="noopener noreferrer">
          <Image alt={`Croak coin`} src={Croak}/>
            Buy $CROAK
          </a>
          <ConnectWallet />
        </div>
      ...
      <main className={styles.main}>
        {data.list.length === 0 && (
          <div className={`${styles.hi} d-flex flex-column align-items-center justify-content-center`}>
            <h1 className={`text-center`}>Hi, I'm <span>Nea</span></h1>
            ...
            data.list.map((item, i) => {
              return (
                <div key={i} className={`${styles.message} ms-motion-slideDownIn`} data-message-type={item.type}>
                  {item.type === `a` && <Image className={styles.logo} src={`/croak.png`} alt="pfp" width={48} height={48} priority />}
          ...
          {isLoading && (
            <>
              <div className={`${styles.message} ms-motion-slideDownIn`} data-message-type={`a`}>
                <Image className={styles.logo} src={`/croak.png`} alt="pfp" width={48} height={48} priority />
